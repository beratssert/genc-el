...
class InstitutionControllerIntegrationTest {
        ...
        void setUp() {
                ...
                userRepository.deleteAll(); // Native query ile soft-deleted dahil temizlenmiyor olabilir; test DB
                                            // create-drop olduğu için genelde sorun olmaz
        ...
        class GetAllInstitutions {
                ...
                void shouldListAllInstitutions() throws Exception {
                        institutionRepository
                                        .save(Institution.builder().name("K1").region("R1").isActive(true).build());
                        institutionRepository
                                        .save(Institution.builder().name("K2").region("R2").isActive(true).build());
        ...
        class InstitutionLogin {
                ...
                void shouldReturn401WhenWrongPassword() throws Exception {
                        Institution inst = institutionRepository
                                        .save(Institution.builder().name("K").region("R").isActive(true).build());
                        User admin = User.builder()
                                        .institution(inst)
                                        .role(User.UserRole.INSTITUTION_ADMIN)
                                        .email("a@b.com")
        ...
        @Nested
        @DisplayName("GET /api/v1/institution/me")
        class GetMyInstitution {

                @Test
                @DisplayName("INSTITUTION_ADMIN kendi kurum detayını alır")
                void institutionAdminCanGetOwnInstitution() throws Exception {
                        mockMvc.perform(get("/api/v1/institution/me")
                                        .with(user(institutionAdmin.getEmail()).roles("INSTITUTION_ADMIN")))
                                        .andExpect(status().isOk())
                                        .andExpect(jsonPath("$.id").value(defaultInstitution.getId().toString()))
                                        .andExpect(jsonPath("$.name").value("Default Kurum"))
                                        .andExpect(jsonPath("$.region").value("Ankara"));
                }

                @Test
                @DisplayName("SYSTEM_ADMIN /me endpoint'ini kullanamaz (403)")
                void superAdminCannotGetViaMeEndpoint() throws Exception {
                        mockMvc.perform(get("/api/v1/institution/me")
                                        .with(user(superAdmin.getEmail()).roles("SYSTEM_ADMIN")))
                                        .andExpect(status().isForbidden());
                }
        ...
        class DeleteMyInstitution {
                ...
                void institutionAdminCanDeleteOwnInstitution() throws Exception {
                        ...
                        org.assertj.core.api.Assertions.assertThat(
                                        institutionRepository.findById(defaultInstitution.getId())).isEmpty();
                }
        ...
        class UpdateAndDeleteInstitutionById {
                ...
                void superAdminCanSoftDeleteInstitution() throws Exception {
                        ...
                        // @SQLRestriction nedeniyle soft-delete sonrası findById, aktif kayıtı
                        // döndürmemeli
                        org.assertj.core.api.Assertions.assertThat(institutionRepository.findById(inst.getId()))
                                        .isEmpty();
                }
